...
import 'package:nocconsumer/cab_service/CabPaymentScreen.dart';
import 'package:nocconsumer/cab_service/cab_payment_selection_screen.dart';
import 'package:nocconsumer/cab_service/dashboard_cab_service.dart';
...
import 'package:nocconsumer/ui/chat_screen/chat_screen.dart';
...
class _CabServiceScreenState extends State<CabServiceScreen> {
  final CameraPosition _kInitialPosition = const CameraPosition(
      target: LatLng(4.2105, 101.9758), zoom: 11.0, tilt: 0, bearing: 0);
  ...
  Future<void> _init() async {
    ...
    FireStoreUtils()
        ...
        .listen((List<User> drivers) {
      updateMarkers(drivers);
    });
  }

  // Function to update markers on the map
  void updateMarkers(List<User> drivers) {
    _markers.clear();
                  ...
                  children: [
                    Positioned(
                      ...
                      right: constraints.maxWidth *
                          0.001, // Adjust the percentage as needed
                      child: Image.asset(
  ...
  void onPressedButton() async {
    ...
    const String phoneNumber = 'tel:999';
  ...
  double getAmount(VehicleType vehicleType) {
    ...
    try {
      double minimumDeliveryWithinKm =
          double.parse(vehicleType.minimum_delivery_charges_within_km!);

      if (distance <= minimumDeliveryWithinKm) {
        totalAmount = double.parse(vehicleType.minimum_delivery_charges!);
      } else {
        totalAmount =
            double.parse(vehicleType.delivery_charges_per_km!) * distance;
      }
    } catch (e) {
      print("Error: $e");
      // Handle the exception (e.g., provide a default value or log the error)
    }
                                ...
                                children: [
                                  Text(
                                      ...
                                      style: const TextStyle(
                                          ...
                                          fontWeight: FontWeight.w600)),
                                  Text(
                                      "EVP No : ${cabOrderModel!.driver!.driverEvpNo}",
                                      style: const TextStyle(
                                          fontFamily: "GlacialIndifference",
                                          fontSize: 18,
                                          fontWeight: FontWeight.w600)),
...
//@#@
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text(
                  "Finding nearest driver as fast\nas we can.\nKindly wait. Thank You.",
                  style: TextStyle(
                      fontFamily: "GlacialIndifference", color: Colors.black),
                  textAlign: TextAlign.center,
                ).tr(),
                const SizedBox(
                  height: 20,
  ...
  void startDialogClosingProcess() {
    // Set the flag to indicate that the dialog closing process has started
    isDialogClosed = true;

    // Start the function to close the dialog after a delay
    closeDialogAfterDelay();
  }

  void stopDialogClosingProcess() {
    // Set the flag to indicate that the dialog closing process should be stopped
    isDialogClosed = false;
  }

  closeDialogAfterDelay() async {
    // Wait for 4 minutes before closing the dialog
    await Future.delayed(Duration(minutes: 5), () {
      // Check if the dialog closing process should continue
      if (isDialogClosed) {
        // Dismiss the dialog
        _cabOrderModel!.status = ORDER_STATUS_REJECTED;
        FireStoreUtils.updateCabOrder(_cabOrderModel!);
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Padding(
            ...
            child: Text(
              "Cancelling a ride will imposed\npenalty to you if done\nrepeatedly.",
              textAlign: TextAlign.center,
              style: TextStyle(
                  fontFamily: "GlacialIndifference",
            ...
            Center(
              child: MaterialButton(
                onPressed: () async {
                  ...
                  _cabOrderModel!.status = ORDER_STATUS_REJECTED;
                  await FireStoreUtils.updateCabOrder(_cabOrderModel!);
  ...
  playSound() async {
    final path =
        await rootBundle.load("assets/audio/noc-customer-alert-tone.mp3");
