...
import 'package:nocconsumer/model/BannerModel.dart';
import 'package:nocconsumer/model/BlockUserModel.dart';
import 'package:nocconsumer/model/BookTableModel.dart';
import 'package:nocconsumer/model/BrandsModel.dart';
import 'package:nocconsumer/model/CabOrderModel.dart';
import 'package:nocconsumer/model/ChatVideoContainer.dart';
import 'package:nocconsumer/model/CodModel.dart';
import 'package:nocconsumer/model/CurrencyModel.dart';
import 'package:nocconsumer/model/DeliveryChargeModel.dart';
import 'package:nocconsumer/model/FavouriteItemModel.dart';
import 'package:nocconsumer/model/FavouriteModel.dart';
import 'package:nocconsumer/model/MercadoPagoSettingsModel.dart';
import 'package:nocconsumer/model/OrderModel.dart';
import 'package:nocconsumer/model/ParcelCategory.dart';
import 'package:nocconsumer/model/PayFastSettingData.dart';
import 'package:nocconsumer/model/ProductModel.dart';
import 'package:nocconsumer/model/Ratingmodel.dart';
import 'package:nocconsumer/model/RentalVehicleType.dart';
...
import 'package:nocconsumer/model/VehicleType.dart';
import 'package:nocconsumer/model/VendorCategoryModel.dart';
import 'package:nocconsumer/model/VendorModel.dart';
import 'package:nocconsumer/model/conversation_model.dart';
import 'package:nocconsumer/model/email_template_model.dart';
import 'package:nocconsumer/model/inbox_model.dart';
import 'package:nocconsumer/model/notification_model.dart';
import 'package:nocconsumer/model/offer_model.dart';
import 'package:nocconsumer/model/paypalSettingData.dart';
import 'package:nocconsumer/model/paytmSettingData.dart';
import 'package:nocconsumer/model/popular_destination.dart';
import 'package:nocconsumer/model/razorpayKeyModel.dart';
import 'package:nocconsumer/model/referral_model.dart';
import 'package:nocconsumer/model/story_model.dart';
import 'package:nocconsumer/model/stripeKey.dart';
import 'package:nocconsumer/model/stripeSettingData.dart';
import 'package:nocconsumer/model/topupTranHistory.dart';
import 'package:nocconsumer/parcel_delivery/parcel_model/parcel_order_model.dart';
import 'package:nocconsumer/parcel_delivery/parcel_model/parcel_weight_model.dart';
import 'package:nocconsumer/rental_service/model/rental_order_model.dart';
...
class FireStoreUtils {
  ...
  Stream<List<User>> getDriversWithinRadius(LatLng? sourceLateLong) {
    var collectionReference =
    firestore.collection(USERS).where("role", isEqualTo: "driver").where("isActive", isEqualTo: true);

    GeoFirePoint center = geo.point(
        latitude: sourceLateLong!.latitude, longitude: sourceLateLong.longitude);

    String field = 'g';

    StreamController<List<User>> driversStreamController =
    ...
    Stream<List<DocumentSnapshot>> stream = geo
        ...
        .within(
      center: center,
      radius: 20, // get drivers only in 20KM radius
      field: field,
      strictMode: true,
    );
    ...
    stream.listen((List<DocumentSnapshot> documentList) {
      ...
      driversStreamController.add(drivers);
    });

    return driversStreamController.stream;
  }
  ...
  static Future<bool> sendChatFcmMessage(
      String title, String message, String token, Map<String, dynamic>? payload) async {
    try {
      var url = 'https://fcm.googleapis.com/fcm/send';
      var header = {
        "Content-Type": "application/json",
  ...
  Future<String?> getStoreNearBy() async {
    DocumentSnapshot<Map<String, dynamic>> codQuery =
        await firestore.collection(Setting).doc('VendorNearBy').get();
    if (codQuery.data() != null) {
  ...
  Stream<List<VendorModel>> getVendors1({String? path}) async* {
    ...
    String field = 'g';
    print(radiusValue.toString() +
        "===RADIUSgetVendors1 ${MyAppState.selectedPosition.latitude} ${MyAppState.selectedPosition.longitude}");
    Stream<List<DocumentSnapshot>> stream = geo
        .collection(collectionRef: collectionReference)
        .within(
            center: center,
...
//   }
  ...
  Future<User?> getUserByIDS(String userId) async {
    ...
    QuerySnapshot<Map<String, dynamic>> vendorsQuery =
        await firestore.collection(USERS).where('id', isEqualTo: userId).get();
    try {
      if (vendorsQuery.docs.isNotEmpty) {
  ...
  static sendOrderEmail({required OrderModel orderModel}) async {
    ...
    newString =
        newString.replaceAll("{coupon}", '(${orderModel.couponCode.toString()})');
    newString = newString.replaceAll(
  ...
  static sendRideBookEmail({required CabOrderModel? orderModel}) async {
    ...
    String newString = emailTemplateModel!.message.toString();
    newString = newString.replaceAll("{passengername}",
        MyAppState.currentUser!.firstName +" "+ MyAppState.currentUser!.lastName);
